...
LOCAL uint8_t state_transition_flag[2]={0U,0U};

LOCAL Can_ControllerStateType Can_ControllerMode [MAX_NUM_OF_CAN_CONTROLLERS];

LOCAL uint8_t IntDisableCount[MAX_NUM_OF_CAN_CONTROLLERS];

LOCAL Can_StateType CanUnitState = CAN_UNINIT ;
...
FUNC(Std_ReturnType,CAN_CODE) Can_SetControllerMode( uint8_t Controller, Can_StateTransitionType Transition )
                {
...
FUNC (Std_ReturnType,CAN_CODE) Can_GetControllerMode( uint8_t Controller, Can_ControllerStateType* ControllerModePtr)
                {
    Std_ReturnType toBeReturned = E_OK;
    uint8_t CanDevolpmentErrorType;
    /*  validating inputs  */
...
FUNC(void,CAN_CODE)  Can_DisableControllerInterrupts(uint8_t Controller)
                {
    uint8_t CanDevolpmentErrorType ;
    if(CanUnitState!=CAN_UNINIT)
    {
...
FUNC(void,CAN_CODE)  Can_EnableControllerInterrupts(uint8_t Controller)
                {
    uint8_t CanDevolpmentErrorType;
    if(CanUnitState!=CAN_UNINIT)
    {
...
FUNC(void,CAN_CODE) CAN_IRQHandler(uint8_t Controller)
                {
...
FUNC(LOCAL void,CAN_CODE) Can_ConfigureHardwareObject(void)
    ...
    uint8_t HO_Index                                              ;
    uint8_t HO_Ref[MAX_NUM_OF_CAN_CONTROLLERS] = {0,0}    ;
    ...
    for ( HO_Index = START_INDEX ; HO_Index < MAX_NUM_OF_HO;HO_Index++ )
    {
        ...
        for( canHardwareObjectIndex = START_INDEX ; canHardwareObjectIndex < NumberMailBoxes_PerHO ; canHardwareObjectIndex++ )
        {
            ...
            if ( CanHardwareObject[HO_Index].CanObjectType == RECEIVE )
            {
                ...
                CANMessage.ui32MsgID     =  0 ;/*CanHwFilterMask[0];*/
...
FUNC(Std_ReturnType,CAN_CODE) Can_SetBaudrate(uint8_t Controller,uint16_t BaudRateConfigID )
                {
    uint8_t CanDevolpmentErrorType ;
    uint8_t ErrorStatus = E_OK;
    uint32_t index8;
...
FUNC(void,CAN_CODE)  Can_Main_Function_Write(void)
                {
...
FUNC(Std_ReturnType,CAN_CODE) Can_GetControllerErrorState( uint8_t ControllerId, Can_ErrorStateType* ErrorStatePtr )
                {
