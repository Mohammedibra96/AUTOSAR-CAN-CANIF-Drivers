 ...
 */
extern CanControllerBaudrateConfig_s  CanControllerBaudrateConfig[MAX_NUM_OF_BAUDRATES ]  ;
/*
 ...
 */
extern uint32_t CanHwFilterMask[MAX_NUM_FILTERS]   ;

/*
 * This container contains the configuration parameters of the CAN controller(s).
 */
extern CanController_s CanController[MAX_NUM_OF_CAN_CONTROLLERS]  ;
/*
 * This container contains the configuration (parameters) of CAN Hardware Objects.
 */
extern CanHardwareObject_s CanHardwareObject[ MAX_NUM_OF_HO ];
...
Can_MailBoxLookUpTables_s Can_MailBoxLookUpTables[MAX_NUM_OF_CAN_CONTROLLERS][MAX_NUM_OF_MAILBOXES];
...
static Can_ControllerStateType Can_ControllerMode [MAX_NUM_OF_CAN_CONTROLLERS];


/*	TODO: ENTER & EXIT CRITICAL SECTION	*/
static uint8_t IntDisableCount[MAX_NUM_OF_CAN_CONTROLLERS];
            ...
            if(((Transition == CAN_T_START) && ( Can_ControllerMode [Controller]==CAN_CS_STOPPED))|| ((Transition == CAN_T_SLEEP) && (Can_ControllerMode [Controller]== CAN_CS_STOPPED)))
            {
                ...
                case CAN_T_STOP :
                    ...
                    for(HwObjectIndex = 0U ; HwObjectIndex < MAX_NUM_OF_HO ; HwObjectIndex++)
                    {
                        ...
                        if( CanHardwareObject[HwObjectIndex].CanObjectType == CAN_OBJECT_TYPE_TRANSMIT)
                        {
...
void  Can_MainFunction_Read(void)
{
    ...
    for ( Controller = CAN_CONTROLLER_ZERO ;Controller <MAX_NUM_CONTROLLER ;Controller++ )
    {
        for ( mailBoxIndex = 0 ; mailBoxIndex < MAX_NUM_OF_HO; mailBoxIndex ++)
        {
            ...
            {
                HwObjectIndex = Can_MailBoxLookUpTables[Controller][mailBoxIndex].HwObject;
                if( CanHardwareObject[HwObjectIndex].CanObjectType == CAN_OBJECT_TYPE_RECEIVE )
                {
...
void CAN_IRQHandler(uint8_t Controller)
{
    uint32_t mailBoxIndex;
    /*	Buffer to recieve data in	*/
    uint8_t rx_MsgData1[8U]={0U};
    ...
    {
        uint8_t HwObjectIndex = Can_MailBoxLookUpTables[Controller][mailBoxIndex].HwObject;
        if( CanHardwareObject[HwObjectIndex].CanObjectType == CAN_OBJECT_TYPE_RECEIVE )
        {
            CANMessageGet(CanController[Controller].CanControllerBaseAddress, mailBoxIndex, &CANMsgObject, 0);
        ...
        else if ( CanHardwareObject[HwObjectIndex].CanObjectType == CAN_OBJECT_TYPE_TRANSMIT )
        {
...
static void Can_ConfigureHardwareObject(void)
{
    ...
    uint8_t HO_Index                                        ;
    uint8_t MailBoxCounter[MAX_NUM_OF_CAN_CONTROLLERS] = {0}    ;
    ...
    for ( HO_Index = START_INDEX ; HO_Index < MAX_NUM_OF_HO;HO_Index++ )
    {
        ...
        CanControllerId  = CanHardwareObject[canHardwareObjectIndex].CanControllerRef                                           ;
        ...
        {
            Can_MailBoxLookUpTables[CanControllerId][MailBoxIndex++].HwObject = MailBoxCounter[CanControllerId];
            if ( StartMailBoxFlag == NEW_HO )
            {
                CanHardwareObject[HO_Index].CanMailBoxStart = HO_Index ;
            ...
            if ( CanHardwareObject[canHardwareObjectIndex].CanObjectType == RECEIVE )
            {
                ...
                CANMessageSet( CanController[CanControllerId].CanControllerBaseAddress, MailBoxCounter[CanControllerId] -1,&CANMessage, MSG_OBJ_TYPE_RX )         ;
            }
            else
            {
                /* MISRA */
...
Std_ReturnType Can_Write(Can_HwHandleType HTH , Can_PduType* PduInfo)
{
    ...
    uint8_t         CanDevolpmentErrorType = NO_ERROR     ;
    ...
    {
        {
            ...
            for( MailBoxIndex = CanHardwareObject[HTH].CanMailBoxStart ; MailBoxIndex < ( CanHardwareObject[HTH].CanHwObjectCount ) ; MailBoxIndex ++)
            {
                ...
                if ( Can_MailBoxLookUpTables[ CanControllerId ][ MailBoxIndex ].Confirmation == CONFIRMED )
                {
                    ...
                    /*Configures a message object in the CAN controller.(TivaWare)                                                          */
                    CANMessageSet( CanController[CanControllerId].CanControllerBaseAddress, MailBoxIndex ,&sCANMessage, MSG_OBJ_TYPE_RX )         ;
                    ErrorStatus            = E_OK     ;
                }
                else
                {
...
void  Can_Main_Function_Write(void)
{
    ...
    for ( CanHwObjectIndex = 0 ; CanHwObjectIndex < MAX_NUM_OF_HO ; CanHwObjectIndex++ )
    {
        for( CanMailBoxIndex = 0U ; CanMailBoxIndex < CanHardwareObject[CanHwObjectIndex].CanHwObjectCount ; CanMailBoxIndex++ )
        {
            if( CanHardwareObject[ CanHwObjectIndex ] .CanObjectType== TRANSMIT )
            {
                if( CanHardwareObject[ CanHwObjectIndex ].CanControllerRef == CAN_CONTROLLER_ZERO )
                {
                ...
                if( Can_MailBoxLookUpTables[CanHardwareObject[ CanHwObjectIndex ].CanControllerRef ][ CanHardwareObject[ CanHwObjectIndex ].CanMailBoxStart + CanMailBoxIndex ].Confirmation == UNCONFIRMED)
                {
                    ...
                    {
                        Can_MailBoxLookUpTables[CanHardwareObject[ CanHwObjectIndex ].CanControllerRef ][ CanHardwareObject[ CanHwObjectIndex ].CanMailBoxStart + CanMailBoxIndex ].Confirmation = CONFIRMED;/*when it comes to poll again it wouldn't find
...
Std_ReturnType Can_GetControllerErrorState( uint8_t ControllerId, Can_ErrorStateType* ErrorStatePtr )
{
    ...
    uint8_t CanDevolpmentErrorType = NO_ERROR;
    static Can_ErrorStateType  ErrorState[ MAX_NUM_OF_CAN_CONTROLLERS];
    if(CanUnitState==CAN_UNINIT)
    {
